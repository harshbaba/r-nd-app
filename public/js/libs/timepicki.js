...
(function($) {
	...
	$.fn.timepicki = function(options) {
		...
		return this.each(function() {
			...
			ele_hei += 10;
			ele.parent().after("<div class='time_pick'>");
			var ele_par = ele.parent().next(".time_pick");
      var $timepicker_wrap = ele_par.find('.timepicker_wrap');

			// developer can specify which arrow makes the numbers go up or down
			var top_arrow_button = (settings.increase_direction === 'down') ?
				'<div class="prev action-prev"><i class="ico-accordian-up"></i></div>' :
				'<div class="prev action-next"><i class="ico-accordian-up"></i></div>';
			var bottom_arrow_button = (settings.increase_direction === 'down') ?
				'<div class="next action-next"><i class="ico-accordian-down"></i></div>' :
				'<div class="next action-prev"><i class="ico-accordian-down"></i></div>';
      ...
      var ele_next = ele_par.find('.timepicker_wrap');
      var ele_next_all_child = ele_next.find("div");
			...
			$(document).on("click", function(event) {
				if (!$(event.target).is(ele_next) && ele_next.css("display")=="block" && !$(event.target).is($('.reset_time'))) {
					if (!$(event.target).is(ele)) {
						set_value(event, !is_element_in_timepicki($(event.target)),true);
					} else {
						var ele_lef =  0;
        ...
        open_timepicki();
			...
			function set_value(event, close,firstTime) {
				var firstTime = typeof firstTime !== "undefined" ? firstTime : false;
